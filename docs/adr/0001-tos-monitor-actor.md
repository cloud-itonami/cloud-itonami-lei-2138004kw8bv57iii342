...
légales") of Publicis Groupe SA, per ADR-2607110300 -- a read-only
reference archive. As part of a fifth 10-repo validation batch extending
the `cloud-itonami-lei-2572ibtt8cczw6au4141` pilot (see
ADR-2607241900/ADR-2607242600 for the full fleet-level design rationale),
this repo gains a governed actor layer on top of the unchanged archive.

This is the first company in this actor family sampled with a
`:legal-notice` doc-type -- the archived page is a French "Mentions
légales" legal notice page, not a terms-of-service page. `:legal-notice`
was already present in `tosmonitor.registry/known-doc-types` (the check
list was designed up front to cover this shape); this is simply the first
company actually sampled that way.
...
- **Company**: Publicis Groupe SA, LEI 2138004KW8BV57III342, website
  `https://www.publicisgroupe.com`.
- **Baseline provenance** (real, from this repo's own `80-data/public/
  tos.journal.edn`): source-url
  `https://www.publicisgroupe.com/fr/mentions-legales`, retrieved-at
  `2026-07-10`, doc-type `:legal-notice`.
...
proves the actor pattern holds for a company whose archived document is a
`:legal-notice` rather than a `:terms-of-service`/`:terms-of-use` page.
